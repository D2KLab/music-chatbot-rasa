...
class MessageProcessor(object):
    ...
    def _handle_message_with_tracker(self, message, tracker):
        ...
        if(tracker.get_slot("requested_slot") is not None):
            from common import check_for_entities
            temp_message = check_for_entities(message.text,tracker.get_slot("requested_slot"))
            if temp_message is None:
                logger.info("Calling default fallback.")
                message.text = INTENT_MESSAGE_PREFIX + "Default Fallback Intent"
            else:
                message.text = INTENT_MESSAGE_PREFIX + tracker.latest_message.intent.get("name") + "[" + tracker.get_slot("requested_slot") + "=" + message.text + "]"
            tracker.update(SlotSet("requested_slot",None))
            logger.info("Modified: {}".format(message.text))
